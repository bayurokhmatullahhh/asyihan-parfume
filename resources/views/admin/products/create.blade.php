...
@section('title', 'Tambah Mahakarya Baru — ASYIHAN Admin')
@section('page_title', 'Persembahkan Mahakarya Baru')
...
<div class="max-w-3xl space-y-6">
    ...
    <div class="bg-[#0e0e0e] border border-gold-400/25 rounded-2xl p-6 sm:p-8 shadow-[0_4px_25px_rgba(0,0,0,0.6)]">
        ...
        <form action="{{ route('admin.products.store') }}" method="POST" class="space-y-5 text-xs">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required
                        ...
                        placeholder="Contoh: ASYIHAN ESSENCE I — SANG PENGGAGAS">
                </div>
                ...
                <div>
                    ...
                    <select name="category" required class="w-full px-4 py-3 bg-black/80 border border-gold-400/25 rounded-xl text-slate-100 focus:outline-none focus:border-gold-400">
                        <option value="essence" {{ old('category') === 'essence' ? 'selected' : '' }}>9 Essence Numerologi</option>
                        <option value="special" {{ old('category') === 'special' ? 'selected' : '' }}>Special Offering</option>
                        <option value="talisman" {{ old('category') === 'talisman' ? 'selected' : '' }}>Talisman / Lilin</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="essence_number" value="{{ old('essence_number') }}" min="1" max="999"
                        class="w-full px-4 py-3 bg-black/80 border border-gold-400/25 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:border-gold-400"
                        placeholder="1">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="tagline" value="{{ old('tagline') }}"
                        class="w-full px-4 py-3 bg-black/80 border border-gold-400/25 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:border-gold-400"
                        placeholder="Aku Tidak Menunggu. Aku Memulai.">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-4 py-3 bg-black/80 border border-gold-400/25 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:border-gold-400"
                        placeholder="Aroma yang diawali dengan kesan segar dari Bergamot...">{{ old('description') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', 350000) }}" required min="0" step="1000"
                ...
                <div>
                    ...
                    <input type="number" name="stock" value="{{ old('stock', 50) }}" required min="0"
                        class="w-full px-4 py-3 bg-black/80 border border-gold-400/25 rounded-xl text-slate-100 focus:outline-none focus:border-gold-400">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="volume" value="{{ old('volume', '50ml') }}" required
                        class="w-full px-4 py-3 bg-black/80 border border-gold-400/25 rounded-xl text-slate-100 focus:outline-none focus:border-gold-400"
                        placeholder="50ml">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="image" value="{{ old('image', 'images/bottle/bottle_1.png') }}"
                        class="w-full px-4 py-3 bg-black/80 border border-gold-400/25 rounded-xl text-slate-100 focus:outline-none focus:border-gold-400">
                </div>
